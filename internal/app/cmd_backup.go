...
func backupFunc(cmd *cobra.Command, args []string) {
	...
	if enable || disable || status || maintenance > 0 {
		...
		case status:
			endpoint += "status"
		case maintenance > 0:
			endpoint += fmt.Sprintf("maintenance/%v", maintenance.Seconds())
		}
	...
	if err == nil && body == "ok" {
		fmt.Println("backup service found... will trigger backup via API...")
		err = ezStream("/api/backup", "")
		if err != nil {
