...
func NewFromEnv() *Client {
	...
	c := Client{
		MetricsTimeout: 1000 * time.Millisecond,
	}
...
func (c Client) GetMetrics() (Metrics, error) {
	...
	topDirs := []string{}
	if !c.looksLikeRepo("") {
		for object := range c.client.ListObjects(ctx, c.Bucket, minio.ListObjectsOptions{UseV1: true}) {
			if object.Err != nil {
	...
	for object := range c.client.ListObjects(ctx, c.Bucket, minio.ListObjectsOptions{Recursive: true, UseV1: true}) {
...
// PutText writes the given string into a file at the given path
func (c Client) PutText(filepath, text string) error {
	ctx, cancel := context.WithTimeout(context.Background(), c.MetricsTimeout)
	defer cancel()
	if _, err := c.client.PutObject(
		ctx, c.Bucket, strings.Trim(filepath, "/"), strings.NewReader(text), -1, minio.PutObjectOptions{
...
// GetText reads a file at the given path and returns it's content as string
func (c Client) GetText(filepath string) (string, bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), c.MetricsTimeout)
	defer cancel()
	obj, err := c.client.GetObject(
		ctx, c.Bucket, strings.Trim(filepath, "/"), minio.GetObjectOptions{},
	)
	if err != nil {
		return "", false, err
	}
