...
func NewFromEnv(opts Opts) *Metrics {
	...
	if m.InfluxdbURL != "" && m.InfluxdbDatabase != "" {
		log.Printf("influxdb support enabled")
		m.influxdbClient = influxdb2.NewClient(m.InfluxdbURL, m.InfluxdbToken)
		m.influxdbWriteAPI = m.influxdbClient.WriteAPIBlocking(m.InfluxdbOrganization, m.InfluxdbDatabase)
	}
	return m.initialize()
}
...
func (m Metrics) initialize() *Metrics {
	...
	//m.influxMeasurements = influxFields{}
	...
	for defName, def := range m.Definition {
		...
		for _, provider := range m.Providers {
			...
			if foundIn != "" {
				log.Fatalf("Metrics: found duplicate field %#v in %v and %T", defName, foundIn, provider.Template)
			}
...
func (m Metrics) apply(pair Pair) {
	...
	if m.InfluxdbEnabled() && pair.Publish {
		...
		point := influxdb2.NewPoint(m.InfluxdbMeasurementPrefix+provider.InfluxdbMeasurement, labels, fields, time.Now())
